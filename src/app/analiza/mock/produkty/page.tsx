import { MockAnalysisShell } from "@/components/analysis/MockAnalysisShell";
import { MOCK_ANALYSIS_PRODUKTY, MOCK_POPUP_DATA_PRODUKTY } from "@/lib/mockData";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";

function CatchlyLogo() {
  return (
    <svg width="80" height="22" viewBox="0 0 99 28" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function MockProduktyPage() {
  return (
    <main className="min-h-screen bg-[#080808] text-white">
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 md:px-12 lg:px-16 py-7 space-y-8">
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center gap-1.5 text-white/30 hover:text-white/70 transition-colors text-sm" style={{ fontFamily: "Satoshi, sans-serif" }}>
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-white/35 text-sm" style={{ fontFamily: "Satoshi, sans-serif" }}>
            Demo — wariant: sklep e-commerce (platinumcandle.pl)
          </p>
        </div>
        <MockAnalysisShell state={MOCK_ANALYSIS_PRODUKTY} popupData={MOCK_POPUP_DATA_PRODUKTY} />
      </div>
    </main>
  );
}
